...
export class TSnake {
  ...
  grow() {
  // Finn det siste kroppselementet (eller head hvis body er tom)
  const lastPart = this.#body.length > 0 ? this.#body[this.#body.length - 1] : this.#head;
  const newBodyPart = lastPart.clone(); // Clone metoden finnes allerede i TSnakeBody

  // Legg til det nye leddet i body-arrayen
