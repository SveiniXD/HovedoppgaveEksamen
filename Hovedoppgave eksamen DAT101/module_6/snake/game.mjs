...
import { EBoardCellInfoType } from "./gameBoard.mjs";
...
export function bateIsEaten() {
  ...
  // Fjern bait fra den gamle cellen
  const baitCell = GameProps.bait.getBoardCell();
  GameProps.gameBoard.getCell(baitCell.row, baitCell.col).infoType = EBoardCellInfoType.Empty;

  // Slangen vokser
  GameProps.snake.grow();

  // Øk farten og lag ny bait
  increaseGameSpeed();
...
function increaseGameSpeed() {
  if (gameSpeed < 15) { // Maks hastighet (du kan justere dette)
    gameSpeed += 0.5;
    clearInterval(hndUpdateGame);
    hndUpdateGame = setInterval(updateGame, 1000 / gameSpeed);
    console.log("New game speed: " + gameSpeed.toFixed(2));
  }
}
